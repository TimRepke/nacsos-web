  ...
  <div>
    ...
    <ul>
      <li v-for="scheme in projectSchemes" :key="scheme.annotation_scheme_id">
        ...
        <ul>
          <li v-for="scope in scopesLookup[scheme.annotation_scheme_id] || []" :key="scope.assignment_scope_id">
            {{ scope.name }}
            <router-link
            ...
            if (confirmationResponse === "ACCEPT") {
              API.core.annotations
                ...
                .then(() => {
                  ...
                  const index = this.projectSchemes.findIndex(
                    (projectScheme: AnnotationSchemeModel) =>
                      projectScheme.annotation_scheme_id === scheme.annotation_scheme_id,
                  );
                  if (index >= 0) {
            ...
            if (confirmationResponse === "ACCEPT") {
              API.core.annotations
                ...
                .then(() => {
                  ...
                  const index = this.projectScopes.findIndex(
                    (assignmentScope: AssignmentScopeModel) =>
                      assignmentScope.assignment_scope_id === scope.assignment_scope_id,
                  );
                  if (index >= 0) {
