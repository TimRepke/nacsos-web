      ...
      <div class="row">
        <div class="col-5 small">
          <ul class="list-unstyled">
            <li><strong>{{ task.status }}</strong></li>
            <li><strong><code>task_id</code></strong>: <code>{{ task.task_id }}</code></li>
            <li><strong><code>user_id</code></strong>: <code>{{ task.user_id }}</code></li>
            <li><strong><code>fingerprint</code></strong>: <code>{{ task.fingerprint }}</code></li>
            <li><strong><code>location</code></strong>: <code>{{ task.location }}</code></li>
            <li><strong><code>function</code></strong>: <code>{{ task.function_name }}</code></li>
            <li><strong><code>started</code></strong>: <code>{{ task.time_started }}</code></li>
            <li><strong><code>finished</code></strong>: <code>{{ task.time_finished }}</code></li>
            <li>marked for deletion: <code>{{ task.rec_expunge }}</code></li>
            <li v-if="task.dependencies">Task dependencies:
              <ul>
...
export default defineComponent({
  ...
  mounted() {
    API.pipe.queue.getTaskApiQueueTaskTaskIdGet({
    ...
    API.pipe.artefacts.getArtefactsApiArtefactsListGet({
  ...
  methods: {
    downloadArchive() {
      API.pipe.artefacts.getArchiveApiArtefactsFilesGet({
        xTaskId: this.taskId,
        xProjectId: currentProjectStore.projectId as string,
      }).then((response) => {
    ...
    downloadFile(filename: string) {
      ...
      API.pipe.artefacts.getFileApiArtefactsFileGet({
        filename,
        xTaskId: this.taskId,
        xProjectId: currentProjectStore.projectId as string,
      }).then((response) => {
    ...
    loadLog() {
      API.pipe.artefacts.getTaskLogApiArtefactsLogGet({
        xTaskId: this.taskId,
        xProjectId: currentProjectStore.projectId as string,
      }).then((response) => {
